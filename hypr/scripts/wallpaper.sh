...
# -----------------------------------------------------
# Check for custom hardcoded colors
# -----------------------------------------------------

CUSTOM_COLORS_FILE="$HOME/.config/ml4w/settings/wallpaper-colors.conf"
wallpaper_basename=$(basename "$used_wallpaper")
custom_colors_found=0

if [ -f "$CUSTOM_COLORS_FILE" ]; then
    # Look for matching wallpaper in config
    while IFS='|' read -r wallpaper primary secondary; do
        # Skip comments and empty lines
        [[ "$wallpaper" =~ ^#.*$ ]] && continue
        [[ -z "$wallpaper" ]] && continue
        
        if [ "$wallpaper" = "$wallpaper_basename" ]; then
            _writeLog "Found custom colors for $wallpaper_basename"
            _writeLog "Primary: $primary, Secondary: $secondary"
            $HOME/.config/ml4w/scripts/apply-custom-colors.sh "$primary" "$secondary"
            custom_colors_found=1
            break
        fi
    done < "$CUSTOM_COLORS_FILE"
fi
...
if [ $custom_colors_found -eq 0 ]; then
    ...
    _writeLog "Skipping matugen - using custom hardcoded colors"
fi
