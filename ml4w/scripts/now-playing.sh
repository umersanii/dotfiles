...
# Strip common source suffixes like " - YouTube Music"
...
# If there's a " by " in the title, extract just the song name
if [[ "$title" == *" by "* ]]; then
    title="${title%% by *}"
fi

# If there's a " - " in the title, take the first part as the song name
if [[ "$title" == *" - "* ]]; then
    title="${title%% - *}"
fi
